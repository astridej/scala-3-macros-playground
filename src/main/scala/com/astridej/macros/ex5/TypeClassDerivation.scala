...
object Eq {
  ...
  given coll[C](using underlying: Eq[C]): Eq[Iterable[C]] =
    new Eq[Iterable[C]] {
      @tailrec
      override def areEqual(c: Iterable[C], d: Iterable[C]): Boolean =
        (c.headOption, d.headOption) match {
          case (None, None) => true
          case (Some(chead), Some(dhead))
              if underlying.areEqual(chead, dhead) =>
            this.areEqual(c.tail, d.tail)
          case _ => false
        }
    }
  ...
  def deriveEqMacro[T: Type](using Quotes): Expr[Eq[T]] = {
    ...
    val fieldCheckEqs: List[(Expr[T], Expr[T]) => Expr[Boolean]] =
              ...
              Expr
                ...
                .getOrElse(
                  report.errorAndAbort(
                    s"Could not find implicit for field $field"
                  )
                )
            (s: Expr[T], t: Expr[T]) =>
              '{
                ${ eq }.areEqual(
                  ${ Select(s.asTerm, field).asExprOf[t] },
                  ${ Select(t.asTerm, field).asExprOf[t] }
                )
              }
        }
      }
...
object MirrorDerivation {
  ...
  given inductionEq[S, T <: Tuple](using sEq: Eq[S], tEq: Eq[T]): Eq[S *: T] =
    (c: S *: T, d: S *: T) =>
      (c, d) match {
        case (s1 *: t1, s2 *: t2) =>
          sEq.areEqual(s1, s2) && tEq.areEqual(t1, t2)
      }
  ...
  inline def deriveEqMirrored[T <: Product](
    ...
    (s: T, t: T) =>
      derivedEq.areEqual(Tuple.fromProductTyped(s), Tuple.fromProductTyped(t))
  }
}
